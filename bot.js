...
const { exec } = require("child_process"); // Импорт exec из child_process

// Объявляем isRestarting как глобальную переменную
let isRestarting = false;

// Замените на ваш токен Telegram-бота
const token = "7446815124:AAH3UirI0n-kFJtaHI4_gndjyB9IkGnODoI";
console.log(`Используется токен бота: ${token}`);
...
- KASSA BALANCE: **${
      totalSum(orders) - totalStop(orders) + 500
    }SEK + 350 EUR + 80 USDT**
        `;
